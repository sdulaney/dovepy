...
#TEST AREA
    ...
    prop = Property('none')
    prop.set_owner_address('1100 S EUCLID ST', 'FULLERTON', 'CA', '92832')
    prop.set_owner_name('RENICK', 'PHIL')

    print(lexis_2.run_crawl(prop))



    ids = data.get_column_data('property_key', file_path)
    names = data.get_column_data('owner_first', file_path)

    for i in range(len(ids)):
        prop = Property(ids[i])
        prop.set_owner_name(names[i], '')
